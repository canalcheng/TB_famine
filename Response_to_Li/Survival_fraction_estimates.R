...
#subset to years where there is data in both censuses
Chinapop = Chinapop[rowSums(is.na(Chinapop))<3,]

Chinapop$Age2000 = 2000 - Chinapop$BirthYear
Chinapop$Age1982 = 1982 - Chinapop$BirthYear
Chinapop$Age1964 = 1964 - Chinapop$BirthYear
...
plot(Chinapop$Age1964,Chinapop$SurvFrac1964_1982,lwd = 2,'l',col = rgb(0,0,0,.5),xlab = 'Initial age',ylab = 'Fraction Surviving', ylim=range(na.omit(Chinapop$SurvFrac1982_2000)))
abline(1,0,lty=2)
lines(Chinapop$Age1982,Chinapop$SurvFrac1982_2000,lwd = 2,col = rgb(0.5,0,0,.5),xlab = 'Birth Year',ylab = 'Fraction Surviving')
text(0,0.6,'Survival 1964-1982',pos = 4,col = rgb(0,0,0,.5))
text(0,0.4,'Survival 1982-2000',pos = 4,col = rgb(0.5,0,0,.5))

#fit bounded isotonic regression for 18 yr survival by age
...
survdiffdat = data.frame(initAge = 0:61, SurvFrac1964_1982.fit = rev(Chinapop$SurvFrac1964_1982.fit[Chinapop$Age1964 %in% (0:61)]),
                         SurvFrac1982_2000.fit = rev(Chinapop$SurvFrac1982_2000.fit[Chinapop$Age1982 %in% (0:61)]))

plot(survdiffdat$initAge,survdiffdat$SurvFrac1982_2000.fit - survdiffdat$SurvFrac1964_1982.fit,lwd = 2,'l',col = rgb(0,0,0,.5),xlab = 'Initial age',ylab = 'Difference in 18 year survival, 1964 vs. 1982')
